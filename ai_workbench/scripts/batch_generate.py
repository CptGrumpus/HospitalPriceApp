...
def process_batch():
    ...
    print("Fetching unique codes from items table...")
    query = text("""
        SELECT 
            code, 
            description 
    ...
    print(f"Found {len(rows)} unique codes to process.")
    ...
    for row in tqdm(rows):
        ...
        existing = session.query(CodeDefinition).filter(CodeDefinition.code == code).first()
        source_text = raw_desc
        
        if existing:
            if existing.generated_title and existing.generated_title != "Unknown Procedure":
                continue # Skip if already good
                
            if existing.long_description:
                source_text = existing.long_description
        else:
            existing = CodeDefinition(code=code)
            session.add(existing)

        if not source_text:
            source_text = "Medical Procedure"
        ...
        try:
            ...
            existing.generated_title = title
            existing.generated_description = desc
            existing.source_text = source_text
            existing.category = "Unknown"
            
            processed_count += 1
        except json.JSONDecodeError:
            continue # Skip if bad JSON
